...
const apiService = {
  createAssets: async (assetsData) => {
    assetsData.userId = userId;
    try {
      const response = await axios.post(`${API_BASE_URL}/assets`, assetsData);
  ...
  getWalletDetails: async () => {
    try {
      const response = await axios.get(
        `${API_BASE_URL}/wallet/get-wallet/${userId}`
      );
      return response.data;
    } catch (error) {
  ...
  getCryptoData: async (cryptoId) => {
    try {
      const response = await axios.get(
        `${API_BASE_URL}/trading/crypto/all-markets`,
        {
          params: { cryptoId },
        }
      );
